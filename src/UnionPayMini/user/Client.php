...
namespace easysdk\UnionPayMini\user;
...
class Client extends UnionPayMiniClient
{
    ...
    public function mobile($code = "", $openId = "", $decrypt = true)
    {
        ...
        if (!isset($result['mobile'])) return $result;

        // 解密返回
        if ($decrypt)
            foreach ($result as $k => $v)
                if ($v)
                    $result[$k] = $this->app->crypto->decrypt($v);
    ...
    public function auth($code = "", $openId = "", $decrypt = true)
    {
        ...
        if ($decrypt)
            foreach ($result as $k => $v)
                if ($v)
                    $result[$k] = $this->app->crypto->decrypt($result[$k]);
    ...
    public function card($code = "", $openId = "", $cardTp = "00", $needSameName = "0", $needPay = "0", $decrypt = true)
    {
        ...
        // 解密返回
        if ($decrypt && !empty($result) && isset($result['cardList']))
            foreach ($result['cardList'] as &$card)
                foreach ($card as $k => $v)
                    if ($v)
